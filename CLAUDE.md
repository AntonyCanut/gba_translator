...
13. **Régressions pointeurs vivants après rebuild** — `make test-rom` inclut
    `make test-fr-rebuild`, qui vérifie les dialogues Pic Cristal, la cutscene de
    la Grotte des Dragons, le panneau de Borrius Est, la victoire du rival et
    l'alignement des offsets ES. Ces tests doivent passer avant tout commit de
    `output/roms/GenedRom-fr.gba`.
14. **Un fix jamais scopé à une seule langue** — `src/core/`, `scripts/build_language.py`
