#!/usr/bin/env python3
"""Translate and relocate the Pokémon-list "Cry" menu-entry label — IT port.
...
Italian translation to free space and repoint both — leaving the untouched
English body where it is.

Idempotent: only writes when both pointers still target the old EN offset.

Usage:
    python3 languages/it/patches/cry_label.py --rom output/roms/GenedRom-it.gba
"""
...
def apply_to_rom(rom: bytearray, dry_run: bool = False) -> int:
    """Write the IT "Grido" body and repoint both cells. Returns patch count."""
...
def main() -> int:
    ...
    print(f"patch_cry_label_it: {n} patch(es) applied{suffix}")
    return 0
