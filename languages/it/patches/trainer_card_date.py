#!/usr/bin/env python3
"""Render the Trainer Card start-of-adventure date in Italian order.
...
i.e. *Month Day, Year* — American order, not the Italian *Day Month Year*.
...
This is the same free-space builder + veneer redirect as the FR/DE ports —
the assembled sequence (day + " " + month + " " + year) and every address it
...
default, or Italian where ``patch_time_format_it.py`` has patched a cell) —
no language-specific logic lives in this script.

Result on screen: "19 Lug. 2026" (once the corresponding month cell has been
translated by ``patch_time_format_it.py``; untranslated months fall back to
their English abbreviation, exactly as in the FR/DE builds).

Every write verifies the bytes it expects and is idempotent.

Usage:
    python3 languages/it/patches/trainer_card_date.py --rom output/roms/GenedRom-it.gba
"""
...
# ---- fixed addresses -------------------------------------------------------
BUILDER_FILE = 0x280120           # free space (verified 0xFF in the IT build)
...
def apply(data: bytearray) -> int:
    ...
    if cur == redirect:
        ...
        # original English/IT block starts with 05 9b 3e 4f ... (ldr r3,[sp,#20])
        if cur[:2] != b"\x05\x9b":
...
def main() -> int:
    ...
    ap.add_argument("--rom", type=Path, default=Path("output/roms/GenedRom-it.gba"))
    args = ap.parse_args()
