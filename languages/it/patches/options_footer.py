#!/usr/bin/env python3
"""Patch the Options-menu page-2 button legend at 0x1F4E244 (Italian).

Port of ``patch_options_footer_fr.py`` — same in-place button-icon legend:

    "<0xF8><0x0A>Scegli <0xF8>ÎCambia <0xF8> <0xF8>ÀOK
     <0xFC>ÀÈ<0xFC>ÂÇ<0xF8>Á<0xF8>ÂExtra"
...
button-icon control code 0xF8 0x0A), so it never reaches ``combined_it.txt``
...
# "Scegli" (Pick) / "Cambia" (Switch) / "OK" (Confirm) / "Extra" (More).
IT_WORDS = ("Scegli", "Cambia", "OK", "Extra")
...
def _build_it_bytes() -> bytes:
    pick, switch, confirm, more = (_encode(w) for w in IT_WORDS)
    return (
...
def apply_patch(rom_path: Path, dry_run: bool = False) -> int:
    ...
    it_bytes = _build_it_bytes()

    if current == it_bytes:
        print("patch_options_footer_it: already applied (0 patch(es))")
        return 0
    ...
    if len(it_bytes) + 1 > SLOT_SIZE:
        print(
            f"  ERROR 0x{OFFSET:07X}: IT bytes ({len(it_bytes)}) exceed slot ({SLOT_SIZE - 1}) — skip",
            file=sys.stderr,
        )
        return 0

    if not dry_run:
        rom[OFFSET : OFFSET + len(it_bytes)] = it_bytes
        rom[OFFSET + len(it_bytes)] = 0xFF
        rom_path.write_bytes(rom)

    print(f"  0x{OFFSET:07X}  options-menu footer legend → IT")
    return 1
...
def main() -> None:
    ...
    print(f"patch_options_footer_it: {n} patch(es) applied{suffix}")
