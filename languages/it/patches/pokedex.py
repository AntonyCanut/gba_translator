#!/usr/bin/env python3
"""Re-wrap every Italian Pokédex description to the 3-line info window.

Port of ``patch_pokedex_de.py``. The Pokédex info screen shows a species
...
language-agnostic logic — it re-wraps whatever text the Italian build already
translated (from ``combined_it.txt`` via the generic pipeline), it does not
carry any Italian-specific wording itself.
...
1. For every Pokédex entry it takes the authoritative Italian text — a
   curated short rewrite from ``languages/it/data/pokedex_it_overrides.json``
...
``languages/it/data/pokedex_it_overrides.json``. The build only aborts on a
...
def load_text_map(translations_path: Path) -> dict:
    """offset -> full Italian translation, from the build's translation JSON."""
...
def apply(
    ...
    for entry in pokedex.iter_entries(source):
        ...
        # Authoritative Italian text: a curated short rewrite first, then the
...
def main() -> int:
    ...
    parser.add_argument("--rom", required=True, help="Built Italian ROM to patch in place")
    ...
    print("✓ Pokédex 3-line rewrap (IT):")
    ...
    if stats["overflow"]:
        ...
        # be shortened later via languages/it/data/pokedex_it_overrides.json.
