#!/usr/bin/env python3
"""Patch the level-up screen stat-change abbreviations to French.
...
Two abbreviations at fixed offsets (0x41B2CC and 0x41B2D4 in the EN ROM):
    Att.Sp.  (Special Attack = Attaque Spéciale)
    Déf.Sp.  (Special Defense = Défense Spéciale)
...
the extractor walks). While they appear in combined_fr.txt for reference,
...
French labels (abbreviated style matching English length):
    Sp. Atk → Att.Sp.  (Attaque Spéciale = Special Attack)
    Sp. Def → Déf.Sp.  (Défense Spéciale = Special Defense)

The patch is idempotent: it accepts either the EN original ("Sp. Atk",
"Sp. Def") or the current FR bytes, so re-running over an already-built
...
_LEVELUP_ABBREVS = [
    {"offset": 0x41B2CC, "en": "Sp. Atk", "fr": "Att.Sp."},
    {"offset": 0x41B2D4, "en": "Sp. Def", "fr": "Déf.Sp."},
]
...
def apply_to_rom(rom: bytearray, dry_run: bool = False) -> int:
    ...
    for entry in _LEVELUP_ABBREVS:
        ...
        # Accept either EN original or current FR
        if current not in {en_text, fr_text}:
            print(
                f"  WARN 0x{offset:06X}: expected one of {en_text!r}, {fr_text!r} "
        ...
        if len(fr_encoded) > len(en_encoded):
            print(
                f"  ERROR 0x{offset:06X}: FR {fr_text!r} ({len(fr_encoded)} bytes) "
        ...
        if not dry_run:
            rom[offset : offset + len(fr_encoded)] = fr_encoded
            # Pad the freed trailing bytes with 0xFF to preserve the terminator region
            for j in range(len(fr_encoded), len(en_encoded)):
                rom[offset + j] = 0xFF

        print(f"  0x{offset:06X}  «{current}» → «{fr_text}»")
        changes += 1
...
def main() -> None:
    ...
    print(f"patch_levelup_stat_abbreviations_fr: {n} patch(es) applied{suffix}")
