#!/usr/bin/env python3
"""Render the Trainer Card "Début de l'aventure" date in French order.

The English game builds the card date inline (function around 0x1EB…, the
block at file 0x1ED8D76) as::

    StringCopy (dest, month_name)      ; "Janv."
    StringAppend(dest, day)            ; "Janv.19"
    StringAppend(dest, ", ")           ; "Janv.19, "
    StringAppend(dest, year)           ; "Janv.19, 2026"

i.e. *Month Day, Year* — American order, not the French *Day Month Year*.
None of it is reachable by the text pipeline (no FDnn date template exists),
and it cannot be reordered in place: r4/r5/r6/r7 are all pinned (0/year/day/
dest) and StringCopy/StringAppend clobber r0-r3, so there is no register left
to spill the month pointer across the calls.

This post-build step therefore:

- writes a small self-contained builder into free space at 0x280120 that
  assembles ``day + " " + month + " " + year`` into the same dest buffer
  (0x02021D18), reusing the day/year strings the original code already
  converted (0x02021CD0 / 0x02021CF0) and the same month pointer table
  (base 0x081FE6BF8, indexed by month);
- rewrites the 44-byte inline block at 0x1ED8D76 to call that builder via the
  existing ``bx r3`` veneer (0x9ED920C), then branch to the print code that
  follows (0x1ED8DA2), leaving r7 = dest for it;
- trims the trailing space from the month-abbreviation cells that carry one
  (Mars/Avr./Mai/Juin/Août/Oct./Nov./Déc.) so the builder's explicit spaces
  don't produce a double space.
...
Usage:
    python3 languages/fr/patches/trainer_card_date.py --rom output/roms/GenedRom-fr.gba
"""
...
# ---- fixed addresses -------------------------------------------------------
BUILDER_FILE = 0x280120           # free space (verified 0xFF)
...
def assemble_builder() -> bytes:
    ...
    # Layout: code, then word-aligned literal pool.
    # We assemble in two passes because ldr/bl need final pool addresses.
    SP_STR_OFF = None  # resolved below

    # First compute code size to know where the pool starts.
    # Instructions (address-independent length): see stream below.
    # code length is fixed; compute pool base.
    CODE_LEN = 0x36  # 54 bytes of code (push..pop) — asserted at the end
    # pool after an alignment nop if needed
    pool_off = CODE_LEN
    if pool_off % 4 != 0:
        pool_off += 2  # nop pad
...
def apply(data: bytearray) -> int:
    ...
    if cur == redirect:
        ...
        # original English/FR block starts with 05 9b 3e 4f ... (ldr r3,[sp,#20])
        if cur[:2] != b"\x05\x9b":
