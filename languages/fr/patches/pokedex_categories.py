#!/usr/bin/env python3
"""Translate Pokédex species-category strings (table 0x1A357CC) to French.
...
Usage (run by ``make build-fr`` after the base injection):
    python3 languages/fr/patches/pokedex_categories.py --rom output/roms/GenedRom-fr.gba
"""
...
def load_mapping(data_dir: Path) -> dict[str, str]:
    path = data_dir / 'pokedex_categories_fr.json'
    with path.open(encoding='utf-8') as fh:
        return json.load(fh)


def apply_patch(data: bytearray, mapping: dict[str, str]) -> tuple[int, int]:
    """Write FR categories into *data* in-place.
    ...
    for idx in range(MAX_ENTRIES):
        ...
        if not en_cat or '[' in en_cat:
            ...
            skipped += 1
            continue
        fr_cat = mapping.get(en_cat)
        if fr_cat is None:
            skipped += 1
            continue
        fr_bytes = _encode(fr_cat)
        # Pad remaining space with 0xFF to clear old bytes
        padded = fr_bytes + b'\xFF' * (CELL_SIZE - len(fr_bytes))
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        '--rom', type=Path, default=Path('output/roms/GenedRom-fr.gba'),
    ...
    print(
        f'Pokédex categories FR patch: {patched} translated, '
