...
1bis. **Nouvelle traduction = test unitaire obligatoire** : chaque ligne non vide ajoutée à `languages/<langue>/combined_<langue>.txt` doit ajouter ou mettre à jour dans le même commit l'entrée exacte (offset et valeur live last-wins) de `languages/<langue>/protected_entries.yaml`. Le hook pré-commit vérifie cette couverture et lance la suite unitaire ; un échec bloque le commit, sans contournement possible.
