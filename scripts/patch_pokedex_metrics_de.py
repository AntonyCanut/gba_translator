#!/usr/bin/env python3
"""Convert Pokédex height/weight displays from imperial to metric (German build).
...
values, not on any text), so they are byte-identical to the French version.
Germany uses the metric system exactly like France, so the same conversion is
needed and the same "kg" unit string applies verbatim.

Only the two-letter field labels differ:
  - "Ht"   -> "Gr"  (Größe = height/size)
  - "Wt"   -> "Ge"  (Gewicht = weight)
  - "lbs." -> "kg"  (identical to French — same unit word in German)
...
PATCHES: list[tuple[int, bytes, bytes]] = [
    ...
    # 8. "Ht\xFF" → "Gr\xFF"  (Größe)
    (0x415F98, b"\xc2\xe8\xff", b"\xc1\xe6\xff"),

    # 9. "Wt\xFF" → "Ge\xFF"  (Gewicht)
    (0x415F9B, b"\xd1\xe8\xff", b"\xc1\xd9\xff"),

    # 10. "lbs.\xFF" → "kg\xFF\x00\x00"  (weight stays numeric kg; identical to French)
