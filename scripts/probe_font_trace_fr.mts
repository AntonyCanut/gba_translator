...
const NICKNAME = new Uint8Array([0x1b, 0x1b, 0x1b, 0x1b, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff]);
...
async function main(): Promise<void> {
  ...
  try {
    ...
    for (let i = 0; i < 600; i++) {
      ...
      const wpId = await client.setRangeWatchpoint(NICK_OFF, NICK_OFF + 10, 'R');
      ...
      fs.writeFileSync(path.join(OUT_DIR, 'ewram.bin'), await readBig(client, 0x02000000, 0x40000));
      fs.writeFileSync(path.join(OUT_DIR, 'iwram.bin'), await readBig(client, 0x03000000, 0x8000));
