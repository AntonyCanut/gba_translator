#!/usr/bin/env python3
"""Patch all status-condition badge graphics to German abbreviations.

Sibling of ``patch_status_badges_fr.py`` — same LZ77 tile layout, different
letter glyphs and slot mapping. The in-battle status badges (PSN, SLP, BRN,
FRZ, PAR, FNT …) are stored as LZ77-compressed 4bpp tile sets.  Each 32-tile
block encodes 8 badge slots of 4 tiles each:
...
the fixed engine order, identical to the one the FR patch relies on):
  slot 0 (pal4,  purple) : PSN  → GIF  (Gift)
  slot 1 (pal6,  yellow) : PAR  → PAR  (unchanged — German keeps « PAR »)
  slot 2 (pal8,  blue)   : SLP  → SCH  (Schlaf)
  slot 3 (pal10, cyan)   : FRZ  → GEF  (Gefroren)
  slot 4 (pal12, red)    : BRN  → VBR  (Verbrennung)
  slot 5 (pal4)          : TOX? → unchanged (garbled / unused)
  slot 6 (pal14, gray)   : FNT  → KO   (fainted, 2-letter badge, K.O.)
  slot 7                 : empty

These 3-letter DE abbreviations (GIF/SCH/GEF/VBR) match the official German
localisation and the text ``status_abbrev`` block in ``languages/de/lang.yaml``.
The KO badge is shared verbatim with the FR patch (« K.O. » is the same in both
languages).
...
(localized block) before we overwrite with German badges.
"""
...
# Glyphs shared with patch_status_badges_fr.py keep the same shapes so both
# language builds render with a consistent badge font.
_LETTERS: dict[str, list[list[bool]]] = {
    # Shared with the FR badge font (identical shapes)
    ...
    # New DE letters designed for 4px-wide, 6-row-tall badge cells
    "I": [[_L,_L,_L,_L],[_B,_L,_L,_B],[_B,_L,_L,_B],[_B,_L,_L,_B],[_B,_L,_L,_B],[_L,_L,_L,_L]],
    "F": [[_L,_L,_L,_L],[_L,_B,_B,_B],[_L,_L,_L,_B],[_L,_B,_B,_B],[_L,_B,_B,_B],[_L,_B,_B,_B]],
    "C": [[_B,_L,_L,_L],[_L,_B,_B,_B],[_L,_B,_B,_B],[_L,_B,_B,_B],[_L,_B,_B,_B],[_B,_L,_L,_L]],
    "H": [[_L,_B,_B,_L],[_L,_B,_B,_L],[_L,_L,_L,_L],[_L,_B,_B,_L],[_L,_B,_B,_L],[_L,_B,_B,_L]],
    "V": [[_L,_B,_B,_L],[_L,_B,_B,_L],[_L,_B,_B,_L],[_L,_B,_B,_L],[_B,_L,_L,_B],[_B,_L,_L,_B]],
}
...
def _patch_block(rom: bytearray, offset: int) -> bool:
    """Decompress block at `offset`, apply all DE status patches, recompress."""
...
def main() -> None:
    ...
    print(f"patch_status_badges_de: {n} block(s) patched{suffix}")
