#!/usr/bin/env python3
"""Patch the Options-menu page-2 button legend at 0x1F4E244.

    "<0xF8><0x0A>Pick <0xF8>ÎSwitch <0xF8> <0xF8>ÀConfirm
     <0xFC>ÀÈ<0xFC>ÂÇ<0xF8>Á<0xF8>ÂMore"
...
button-icon control code 0xF8 0x0A), so it never reaches `combined_fr.txt` /
the injection JSON — same class as `patch_status_abbrevs_fr.py`. No pointer
to it was found by scanning the ROM for its absolute address either, so it
cannot be safely relocated: this is a direct in-place patch, tightly packed
against the next string (0x1F4E26F), leaving 42 content bytes + 0xFF
...
FR_WORDS = ("Choix", "Changer", "OK", "Plus")
...
def _build_fr_bytes() -> bytes:
    pick, switch, confirm, more = (_encode(w) for w in FR_WORDS)
    return (
...
def apply_patch(rom_path: Path, dry_run: bool = False) -> int:
    ...
    fr_bytes = _build_fr_bytes()

    if current == fr_bytes:
        print("patch_options_footer_fr: already applied (0 patch(es))")
        return 0
    ...
    if len(fr_bytes) + 1 > SLOT_SIZE:
        print(
            f"  ERROR 0x{OFFSET:07X}: FR bytes ({len(fr_bytes)}) exceed slot ({SLOT_SIZE - 1}) — skip",
            file=sys.stderr,
        )
        return 0
    ...
    if not dry_run:
        ...
        rom[OFFSET + len(fr_bytes)] = 0xFF
        rom_path.write_bytes(rom)

    print(f"  0x{OFFSET:07X}  options-menu footer legend → FR")
    return 1
...
def main() -> None:
    ...
    print(f"patch_options_footer_fr: {n} patch(es) applied{suffix}")
