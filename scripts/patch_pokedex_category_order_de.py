#!/usr/bin/env python3
"""Swap the Pokédex category/«Pokémon» order to the official German form (German).
...
English layout. German Pokédex conventions put the brand name first, exactly
like French: ``Pokémon Mausratte``. Since "Pokémon" is spelled identically in
German (it is not a translated word — the brand name is unchanged across every
localisation), this patch is functionally identical to the French one: same
instruction swap, same fixed suffix bytes (``Pokémon`` already lives at
0x415F8F in every generic build, untouched by the translation pipeline).
