#!/usr/bin/env python3
"""Translate and relocate the trainer Pokémon recall strings (come-back messages) — DE port.

Same mechanism as ``patch_battle_recall_strings_fr.py`` (tickets B-78 / B-81).

``patch_battle_string_templates_de.py`` restores the whole cluster
...
0x3FE510``). Because the pointers are external, we can relocate the German
...
German translations
--------------------
  {FD1D}: {FD06}, komm zurück!
  {FD1D}: {FD08}, komm zurück!
  {FD1D}: {FD06} und\\n{FD08}, kommt zurück!
...
DE_STRINGS: list[bytes] = [
    _fd(0x1D) + _enc(": ") + _fd(0x06) + _enc(", komm zurück!") + bytes([_TERMINATOR]),
    _fd(0x1D) + _enc(": ") + _fd(0x08) + _enc(", komm zurück!") + bytes([_TERMINATOR]),
    (
        _fd(0x1D) + _enc(": ") + _fd(0x06) + _enc(" und") + bytes([_NEWLINE])
        + _fd(0x08) + _enc(", kommt zurück!") + bytes([_TERMINATOR])
    ),
]
...
def apply_to_rom(rom: bytearray, dry_run: bool = False) -> int:
    """Write the DE recall strings and repoint the table. Returns patch count."""
    changes = 0
    for (ptr_off, old_body_off), de_bytes in zip(RECALL_PTRS, DE_STRINGS):
        ...
        if ptr != expected_ptr:
            ...
            # Check if the current target starts with the German string.
            cur_off = ptr - GBA_BASE
            if 0 < cur_off < len(rom) - len(de_bytes):
                if rom[cur_off: cur_off + len(de_bytes)] == de_bytes:
        ...
        new_off = _find_free(rom, len(de_bytes))
        if new_off is None:
            print(f"  FAIL @0x{ptr_off:06X}: no free space for {len(de_bytes)} bytes", file=sys.stderr)
            continue

        if not dry_run:
            rom[new_off: new_off + len(de_bytes)] = de_bytes
        ...
        print(
            ...
            f"recall string #{changes} relocated to 0x{new_off:06X} ({len(de_bytes)} bytes)"
        )
...
def main() -> None:
    ...
    print(f"patch_battle_recall_strings_de: {n} patch(es) applied{suffix}")
