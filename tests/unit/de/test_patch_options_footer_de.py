...
injection JSON and needs this dedicated in-place patch.
"""
...
class TestPatchOptionsFooterDe(unittest.TestCase):
    ...
    def test_skips_unexpected_current_value(self):
        rom = _seed_rom(b"\x00" * len(EN_ORIGINAL))  # neither EN nor DE
        n, patched = _apply_on(rom)
        self.assertEqual(n, 0)

    def test_every_de_word_encodes(self):
        for word in mod.DE_WORDS:
            for ch in word:
