...
def _make_rom() -> bytearray:
    """Synthetic ROM large enough to hold both icon-sheet copies, with each
    icon's pill rows (8-9) filled with a recognisable per-type colour and the
    text band set to that colour (so 'English' text == blank pill)."""
    size = max(mod.BASES) + 0xC0 * 32
    rom = bytearray(size)
