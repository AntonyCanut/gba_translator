...
class ReinserterRelocationTests(unittest.TestCase):
    ...
    def test_relocation_failure_falls_back_to_in_place_shrink(self):
        """#54: when a queued relocation finds no free space anywhere, the
        entry must not silently keep the original English text — it should
        shrink to fit its own original cell instead, same as the immediate
        fallback path used when relocation isn't even attempted."""
        # No large 0xFF run anywhere: any relocation attempt is bound to fail.
        rom = bytearray([0xAB] * 0x4000)
        original_offset = 0x20
        rom[0:4] = struct.pack('<I', 0x08000000 + original_offset)
        ...
        long_translation = 'Hello there, this text is much too long to fit'
        translation = {
            'offset': original_offset,
        ...
        reinserter = SmartReinserter(rom, allow_relocate=True, allow_fallback=True)
        success = reinserter.reinsert_text(translation)
        self.assertTrue(success)
        reinserter.flush_relocations()

        report = reinserter.get_report()['statistics']
        self.assertEqual(report['relocation_failed'], 0)
        self.assertEqual(report['fallback_used'], 1)

        # The pointer was never touched (nothing to relocate to), and the
        # original cell now holds a shrunk-but-translated string, not English.
        pointer = struct.unpack('<I', rom[0:4])[0] - 0x08000000
        self.assertEqual(pointer, original_offset)
        self.assertNotEqual(bytes(rom[original_offset:original_offset + len(original)]), original)

    def test_relocation_failure_without_fallback_still_reports_failure(self):
        """Without --allow-fallback, a relocation that finds no space must
        still be reported as a genuine failure (no silent fallback)."""
        rom = bytearray([0xAB] * 0x4000)
        original_offset = 0x20
        rom[0:4] = struct.pack('<I', 0x08000000 + original_offset)
        original = TextEncoder.encode_pokemon('Hi')
        ...
        translation = {
            'offset': original_offset,
            'translation': 'Hello there, this text is much too long to fit',
            'encoding': 'pokemon',
            'original_length': 2,
            'padding_available': 0,
            'pointer_offsets': [0],
        ...
        reinserter = SmartReinserter(rom, allow_relocate=True, allow_fallback=False)
        reinserter.reinsert_text(translation)
        reinserter.flush_relocations()

        report = reinserter.get_report()['statistics']
        self.assertEqual(report['relocation_failed'], 1)
        self.assertEqual(report['fallback_used'], 0)
