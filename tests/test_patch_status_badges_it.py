"""Regression tests for the IT status-badge graphics patch.

The Italian build must render the official IT status abbreviations as badge
tiles: SON (Sonno), CON (Congelato), SCT (Scottatura) and the 2-letter KO
(fainted); PSN and PAR are left untouched (identical to the English art).
These match the text ``status_abbrev`` block in ``languages/it/lang.yaml``.
Verified at the pixel level by regenerating the tiles the patch produces and,
when the ROM is built, by decompressing the built ROM's badge block and
comparing slot tiles.
"""
...
class TestBadgePatchTable(unittest.TestCase):
    def test_slot_mapping_matches_official_it_abbrevs(self):
        # The badge tiles must spell exactly the official IT abbreviations.
        expected = {
            2: ("S", "O", "N"),   # SLP → SON
            3: ("C", "O", "N"),   # FRZ → CON
            4: ("S", "C", "T"),   # BRN → SCT
    ...
    def test_psn_and_par_slots_left_untouched(self):
        # PSN and PAR are identical in Italian, so slots 0 and 1 must not be patched.
        touched = {slot for slot, *_ in _STATUS_PATCHES}
        self.assertNotIn(0, touched)
    ...
    def test_badges_match_registry_status_abbrev(self):
        ...
        abbrev = load_registry().get("it").status_abbrev
...
@pytest.mark.rom
class TestBuiltItBadge(unittest.TestCase):
    """The shipped IT ROM's badges must contain the IT-abbreviation tiles."""
    ...
    def test_status_badges_render_it_abbrevs(self):
